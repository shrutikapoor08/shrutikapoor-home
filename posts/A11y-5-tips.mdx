---
title: Creating an accessible app in 5 simple steps
...
Using semantic HTML is important for accessibility. The reason is that assistive technologies such as screen readers are able to interpret what's on the page by looking at the HTML of the page. They enable users to take actions based on the elements. For eg, if a screen reader encounters `button`, it would signal the user to click on it. Let's consider some use-cases of what happens when semantic HTML is not used -

1. Creating buttons by using `div` instead of `button`: `div` are container elements so when a screen reader encounters a div, it automatically assigns a role of `group` to it. When a screen reader user encounters a `div`, screen reader would announce `role="group"` and the user will completely miss that the `div` is interactive.
...
Adding a `tabindex` is a very widely used method to make interactive elements keyboard navigable. When `tabindex` is added to an element, a user is able to keyboard navigate to it. A caution though - `tabindex` should only be added to interactive elements such as `button`, `a`.
`tabindex` should only have a value of `0` or `-1`. Do not assign a value of > 1 to tabindex.
